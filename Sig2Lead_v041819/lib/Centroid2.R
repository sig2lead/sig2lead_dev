...
#######################################Changed 04-16-2019 ###########################
      ...
      #cluster_info <- as.data.frame(cbind(Centroid=unlist(centroid), ClusterSize=as.numeric(unlist(cluster_counts)), Added=ifelse(!is.null(Added_Counts), as.numeric(unlist(Added_Counts)),0), stringsAsFactors=FALSE)) #P2Count=as.numeric(unlist(P2_counts)), P4Count=as.numeric(unlist(P4_counts))), stringsAsFactors = FALSE)
      ...
      cluster_info$LINCS <- LINCSFreq
      #colnames(cluster_info) <- c("Centroid", "ClusterSize", "Added", "LINCS", "x1", "y1", "radius")
      cluster_info <<- cluster_info
    }
  }
}
