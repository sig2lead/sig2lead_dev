...
#      }
  ...
  ##################################################
      ...
      #added_sims_length <- length(sdfset_add)
      #length_lsm_and_added <- length(fpset)
         
  
      #added_sims <- simMA[1:(length_lsm_and_added - added_sims_length),((lsm_sims+1):length_lsm_and_added)]
      added_sims <- simMA[1:(length(fpset) - length(sdfset_add)),((length(fpset) - length(sdfset_add))+1):length(fpset)]
      #added_sims <- simMA[1:(length(fpset) - length(sdfset_add)),((length(fpset) - length(sdfset_add))+1):length(fpset)]
      #added_sims <- simMA
      #added_sims <- simMA
  ...
  concordance_vector <<- as.vector(concordance[,3])
  
  concordance_df <- concordance[, c(1,3)]
  concordance_df$`LSM-ID` <- as.character(concordance_df$`LSM-ID`)
...
#-032919------------------------------------------------------------------------------  
  ...
  library(dplyr)
  added_sims_df_no_dups <- added_sims_df[,!duplicated(colnames(added_sims_df))]
  score_df2 <- left_join(added_sims_df_no_dups, concordance_df, by = "LSM-ID")
  

    
#score_df3 <- score_df2[ , 1:10]
score_df3 <<- score_df2[ , 1:(ncol(score_df2)-2)]
 rownames(score_df3) <- score_df2$`LSM-ID`
 score_df3b <- score_df3
...
# score_df <- score_df4[, 1:10]
 ...
 z <- list()
 ind <- vector("integer", length=(ncol(score_df) -1))
 for(i in 1:(ncol(score_df)-1)){
 ...
 colnames(z_df) <- c("My Compounds", "LSM-ID", "Concordance", "Similarity", "Combined Score")
