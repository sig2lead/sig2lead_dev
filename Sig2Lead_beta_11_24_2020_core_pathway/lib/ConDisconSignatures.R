...
ConDisconSignatures <- function(signatureID, activate_inhibit, threshold)
  {
...
if(activate_inhibit == "Inhibit"){
  con <<- df2[df2$similarity > threshold,]
  #discon <- df2[df2$similarity < -0.235, ]
  #discon <- df2[df2$similarity < input$Concordance, ]
}
else if(activate_inhibit == "Activate"){
  con <<- df2[df2$similarity < threshold,]
  #discon <- df2[df2$similarity < -0.235, ]
  #discon <- df2[df2$similarity < input$Concordance, ]
}
