...
#  output$Go <- eventReactive(input$Go,  
    {
      ...
      #1. Query iLINCS for user input gene
        if (input$Signature == "Define Target Gene"){
          ...
          #ConDisconSignatures(sigid,input$ConOrDiscon)#}
          
          #ConDisconSignatures <<- ConDisconSignatures(sigid)
          #ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon)
          ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon, input$Concordance)
          ##############################################################
          ##############################################################
        ...
        ########################################################################################################
        # Set of Target Genes
        else if (input$Signature == "Define Set of Target Genes"){          #shinyjs::hideElement("CANDIDATES")
          load("./lincs_fps.RData")
          source("lib/GetSMILES.R", local = TRUE)
          source("lib/ChemmineOB.R", local = TRUE)
          if(!is.null(input$gene_knockdown_1)){
          shinyjs::hideElement("check_kd")
          #removeUI(selector="output$CANDIDATES")
          #emoveUI(selector="output$SMILES")
          ...
          #if(input$ConOrDiscon=="Concordant"){
           # incProgress(1/7, message = "Finding Concordant Signatures")
           #  ConDisconSignatures(sigid, input$ConOrDiscon_1)#}
          
          #ConDisconSignatures <<- ConDisconSignatures(sigid)
               ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon_1, input$threshold_1)
          ...
          max_cons_2 <- max_cons[!duplicated(max_cons$`LSM-ID`),]
          max_cons_3 <- max_cons_2[order(max_cons_2$Concordance, decreasing = TRUE), -5]
          max_cons_4_1 <- max_cons_3[-which(is.na(max_cons_3$`LSM-ID`)),]
          con_df <- as.data.frame((matrix(unlist(con_comma),byrow=T)))
          max_cons_4_1$Concordance <- round(max_cons_4_1$Concordance,3)
          max_cons_4_1 <<- max_cons_4_1
          lincs_output <- max_cons_4_1
          colnames(lincs_output) <- c("Candidate LSM ID", "Candidate Name", "Cell Line", "Concordance", "Significance")
          lincs_output <- lincs_output[,-5]
          lincs_output <- lincs_output[,c(2,1,3,4)]
          shinyjs::showElement("SMILES")
          ...
          #  lincs_output
          #  return(lincs_output)}, caption="LINCS Candidates Ranked", escape=FALSE, rownames=FALSE)
          lincs_output_1 <<- lincs_output
          #load("./lincs_fps.RData")
          # 
          lsm_rows_1 <<- which(rownames(lincs_fps_2) %in% max_cons_4_1$`LSM-ID`)
          ...
          if(!(input$gene_knockdown_2=="")){
            ...
            print("Found your knockdowns") 
            #if(input$ConOrDiscon=="Concordant"){
              incProgress(1/7, message = "Finding Signatures")
              #ConDisconSignatures(sigid)#}
            #  ConDisconSignatures(sigid, input$ConOrDiscon_2)#}
            
            ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon_2, input$threshold_2)
            ##############################################################
            ##############################################################
            # Redo with LINCS fingerprints loaded
            #load("./lincs_fps.RData")
            ...
            max_cons_2 <- max_cons[!duplicated(max_cons$`LSM-ID`),]
            max_cons_3 <- max_cons_2[order(max_cons_2$Concordance, decreasing = TRUE), -5]
            max_cons_4_2 <- max_cons_3[-which(is.na(max_cons_3$`LSM-ID`)),]
            con_df <- as.data.frame((matrix(unlist(con_comma),byrow=T)))
            max_cons_4_2$Concordance <- round(max_cons_4_2$Concordance,3)
            max_cons_4_2 <<- max_cons_4_2
            lincs_output <- max_cons_4_2
            colnames(lincs_output) <- c("Candidate LSM ID", "Candidate Name", "Cell Line", "Concordance", "Significance")
            lincs_output <- lincs_output[,-5]
            lincs_output <- lincs_output[,c(2,1,3,4)]
            shinyjs::showElement("SMILES")
            ...
            #  lincs_output
            #  return(lincs_output)}, caption="LINCS Candidates Ranked", escape=FALSE, rownames=FALSE)
            lincs_output_2 <<- lincs_output
            #load("./lincs_fps.RData")
            # 
            lsm_rows_2 <<- which(rownames(lincs_fps_2) %in% max_cons_4_2$`LSM-ID`)
          ...
          if(!(input$gene_knockdown_3 == "")){
            ...
            #ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon_3)
            ConDisconSignatures <<- ConDisconSignatures(sigid, input$ConOrDiscon_3, input$threshold_3)
            ##############################################################
            ##############################################################
            # Redo with LINCS fingerprints loaded
            #load("./lincs_fps.RData")
            ...
            max_cons_2 <- max_cons[!duplicated(max_cons$`LSM-ID`),]
            max_cons_3 <- max_cons_2[order(max_cons_2$Concordance, decreasing = TRUE), -5]
            max_cons_4_3 <- max_cons_3[-which(is.na(max_cons_3$`LSM-ID`)),]
            con_df <- as.data.frame((matrix(unlist(con_comma),byrow=T)))
            max_cons_4_3$Concordance <- round(max_cons_4_3$Concordance,3)
            max_cons_4_3 <<- max_cons_4_3
            lincs_output <- max_cons_4_3
            colnames(lincs_output) <- c("Candidate LSM ID", "Candidate Name", "Cell Line", "Concordance", "Significance")
            lincs_output <- lincs_output[,-5]
            lincs_output <- lincs_output[,c(2,1,3,4)]
            #shinyjs::showElement("SMILES")
            ...
            #  lincs_output
            #  return(lincs_output)}, caption="LINCS Candidates Ranked", escape=FALSE, rownames=FALSE)
            lincs_output_3 <<- lincs_output
            #load("./lincs_fps.RData")
            # 
            lsm_rows_3 <<- which(rownames(lincs_fps_2) %in% max_cons_4_3$`LSM-ID`)
          ...
          index <<- 1
          index_analogs <<- 2
          if(input$gene_kd_union == "Intersection"){
            temp <- rbind(display_1, display_2)
            temp <- temp[which(duplicated(temp[,1])),]
            if(!(input$gene_knockdown_3 == "")){
            temp <- rbind(temp,display_3)
            temp <- temp[which(duplicated(temp[,1])),]
            }
            final_compounds <- temp
            
            #var<-as(final_compounds$SM_SMILES_Parent, "SMIset")
            #cid(var)<-final_compounds$SM_LINCS_ID
            
            colnames(final_compounds) <- c("SM_LINCS_ID", "SM_SMILES_Parent")
            final_compounds <<- final_compounds
            Get_SDF()
            
            lincs_output <- rbind(lincs_output_1, lincs_output_2)
            lincs_output <- lincs_output[order(abs(lincs_output$Concordance), decreasing=FALSE),]
            lincs_output <- lincs_output[which(duplicated(lincs_output$`Candidate LSM ID`)),]
            max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2)
            max_cons_4 <- max_cons_4[order(abs(max_cons_4$Concordance), decreasing=FALSE),]
            max_cons_4 <- max_cons_4[which(duplicated(max_cons_4$`LSM-ID`)),]
            #max_cons_4 <- max_cons_4[order(max_cons_4$`LSM-ID`, max_cons_4$Concordance),]
            #max_cons_4 <- max_cons_4[-which(duplicated(max_cons_4$`LSM-ID`)),]
            
              if (!(input$gene_knockdown_3 == "")){
                lincs_output <- rbind(lincs_output, lincs_output_3)
                lincs_output <- lincs_output[order(abs(lincs_output$Concordance), decreasing=FALSE),]
                lincs_output <- lincs_output[which(duplicated(lincs_output$`Candidate LSM ID`)),]
                #lincs_output <- lincs_output[duplicated(lincs_output$`Candidate LSM ID`),]
                #lincs_output <- lincs_output[duplicated(max_cons_4$`LSM-ID`),]
                lincs_output <- lincs_output[order(abs(lincs_output$Concordance), decreasing=TRUE),]
                #lincs_output <- lincs_output[-which(duplicated(lincs_output$`Candidate LSM ID`),]
                
                lincs_output <<- lincs_output
                #lsm_rows <- c(lsm_rows_1, lsm_rows_2, lsm_rows_3)
                lsm_rows <- c(lsm_rows_1, lsm_rows_2)
                lsm_rows <- lsm_rows[duplicated(lsm_rows)]
                lsm_rows <- c(lsm_rows,lsm_rows_3)
                lsm_rows <- lsm_rows[duplicated(lsm_rows)]
                lsm_rows <<- lsm_rows
                #max_cons_4 <- lincs_output
                #max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2, max_cons_4_3)
                max_cons_4 <- rbind(max_cons_4 ,max_cons_4_3)
                max_cons_4 <- max_cons_4[order(abs(max_cons_4$Concordance), decreasing=FALSE),]
                max_cons_4 <- max_cons_4[which(duplicated(max_cons_4$`LSM-ID`)),]
                #max_cons_4 <- max_cons_4[duplicated(max_cons_4$`LSM-ID`),]
                max_cons_4 <- max_cons_4[order(abs(max_cons_4$Concordance), decreasing=TRUE),]
                #max_cons_4 <- max_cons_4[-which(duplicated(max_cons_4$`LSM-ID`)),]
                
                #max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2, max_cons_)
                ...
                #lincs_output <- rbind(lincs_output_1, lincs_output_2)
                #lincs_output <- lincs_output[order(lincs_output$Concordance, decreasing=FALSE),]
                #lincs_output <- lincs_output[which(duplicated(lincs_output$`Candidate LSM ID`)),]
                #lincs_output <- lincs_output[duplicated(lincs_output$`Candidate LSM ID`),]
                #lincs_output <- lincs_output[duplicated(max_cons_4$`LSM-ID`),]
                #lincs_output <- lincs_output[order(lincs_output$Concordance, decreasing=TRUE),]
                #lincs_output <- lincs_output[-which(duplicated(lincs_output$`Candidate LSM ID`),]
                ...
                #max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2, max_cons_4_3)
                #max_cons_4 <- rbind(max_cons_4 ,max_cons_4_3)
                #max_cons_4 <- max_cons_4[order(max_cons_4$Concordance, decreasing=FALSE),]
                #max_cons_4 <- max_cons_4[which(duplicated(max_cons_4$`LSM-ID`)),]
                #max_cons_4 <- max_cons_4[duplicated(max_cons_4$`LSM-ID`),]
                #max_cons_4 <- max_cons_4[order(max_cons_4$Concordance, decreasing=TRUE),]
                #max_cons_4 <- max_cons_4[-which(duplicated(max_cons_4$`LSM-ID`)),]
                ...
                #max_cons_4 <- lincs_output
                #max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2)
                #max_cons_4 <- max_cons_4[duplicated(max_cons_4),]
                #max_cons_4 <- max_cons_4[duplicated(max_cons_4$'LSM-ID'),]
                #max_cons_4 <<- max_cons_4
                #max_cons_4 <<- max_cons_4
                print("Made it this far.  In gene_knockdown_3")
          ...
          else if(input$gene_kd_union == "Union"){
            
            if(!(input$gene_knockdown_3 == "")){
              temp <- rbind(display_1,display_2, display_3)
              temp <- temp[which(duplicated(temp[,1])),]
            }
            else{
              temp <- rbind(display_1, display_2)
            ...
            if(!(input$gene_knockdown_3 == "")){
              lincs_output <- rbind(lincs_output_1, lincs_output_2, lincs_output_3)
              #lincs_output <- distinct(lincs_output)
              lincs_output <- lincs_output[order(abs(lincs_output$Concordance), decreasing=TRUE),]
              lincs_output <- lincs_output[-which(duplicated(lincs_output$`Candidate LSM ID`)),]
              #lincs_output <- distinct(lincs_output)
              
              lsm_rows <- c(lsm_rows_1, lsm_rows_2, lsm_rows_3)
              lsm_rows <- unique(lsm_rows)
              lsm_rows <<- lsm_rows
              #max_cons_4 <- lincs_output
              max_cons_4 <- rbind(max_cons_4_1,max_cons_4_2, max_cons_4_3)
              max_cons_4 <- max_cons_4[order(abs(max_cons_4$Concordance), decreasing=TRUE),]
              ...
              lincs_output <- rbind(lincs_output_1, lincs_output_2)
              #lincs_output <- unique(lincs_output)
              lincs_output <- lincs_output[order(abs(lincs_output$Concordance), decreasing=TRUE),]
              lincs_output <- lincs_output[-which(duplicated(lincs_output$`Candidate LSM ID`)),]
              lsm_rows <- c(lsm_rows_1, lsm_rows_2)
              lsm_rows <- unique(lsm_rows)
      ...
      #5.2. Add compounds, but bypass clustering
        else
                {
                  ...
                  if(!(grepl(".sdf", adds$name))){
                    ...
                    else if ((input$Algorithm == "minLSH") & (input$Signature == "Define Target Gene")){}
                    
                    
                    else if ((input$Algorithm == "minSim") & (input$Signature == "Define Set of Target Genes")){}
                    
                    else if ((input$Algorithm == "fpSim") & (input$Signature == "Define Set of Target Genes")){}
                    
                    else if ((input$Algorithm == "minLSH") & (input$Signature == "Define Set of Target Genes")){}
                    ...
                    if ((input$Algorithm == "fpSim") & (input$Signature == "Define Set of Target Genes")){
                      adds<<-input$AddedCompounds
                      sdfset_add <- read.SDFset(paste(getwd(), adds$name, sep="/"))
                      added_labels <- sdfid(sdfset_add)
                    ...
                    if ((input$Algorithm == "minLSH") & (input$Signature == "Define Set of Target Genes")){}
                    ...
                    { #load("./minSim_apfp_RObjects.RData")
                      ...
                      test_bypass_minlsh <- minlsh(sdfset_add, 64,4,lsm_rows,lincs_fps_2)
                      
                      print("This is after minlsh")
                      test_bypass_minlsh <- unlist(test_bypass_minlsh)
                      test_bypass_minlsh <- data.frame(names(test_bypass_minlsh), unlist(test_bypass_minlsh))
                      #test_bypass_minlsh <- data.frame(LSM = names(test_bypass_minlsh), sapply(test_bypass_minlsh, length)), Similarity = unlist(test_bypass_minlsh))
                      test_keep <<- test_bypass_minlsh
                      test_bypass_minlsh <- data.frame(added_labels, test_bypass_minlsh)
                      
                      colnames(test_bypass_minlsh) <- c("Compound", "LSM-ID", "Similarity")
                      Concordance <- vector("numeric", length=nrow(test_bypass_minlsh))
                      Cell_Line <- vector("character", length=nrow(test_bypass_minlsh))
                      test_bypass_minlsh <- left_join(test_bypass_minlsh, max_cons_4, by = "LSM-ID")
                      test_bypass_minlsh <- test_bypass_minlsh[, c(1, 2, 5, 6, 3)]
                      test_bypass_minlsh$Concordance <- round(test_bypass_minlsh$Concordance, 3)
                      test_bypass_minlsh$Similarity <- round(test_bypass_minlsh$Similarity, 3)
                      ...
                      #test_bypass_minlsh <- test_bypass_minlsh[,c("Compound", "LSM_ID", "Cell_Line", "Concordance","Similarity")]
                      colnames(test_bypass_minlsh) <-c("User-added Compound", "LINCS Analog", "Cell Line", "Concordance", "Similarity")
                      #output$CANDIDATES <- DT::renderDataTable(test_bypass_minsim[order(test_bypass_minsim[,5], test_bypass_minsim[,4], decreasing=TRUE),], caption="My Candidates Ranked", rownames=FALSE)
                      #test_bypass_minsim <- test_bypass_minsim[order(test_bypass_minsim[,5], test_bypass_minsim[,4], decreasing=TRUE),]
                      ...
                      if (input$ConOrDiscon == "Inhibit"){
                        print("Inhibit")
                        test_bypass_minlsh <- test_bypass_minlsh[order(-test_bypass_minlsh$Similarity, -test_bypass_minlsh$Concordance),]
                      }
                      ...
                      shinyjs::showElement("CANDIDATES")
                      output$CANDIDATES <- DT::renderDataTable({
                       test_bypass_minlsh$`LINCS Analog`<- makeLink5(test_bypass_minlsh$`LINCS Analog`)
                      #test_bypass_minlsh[order(test_bypass_minlsh$Similarity, test_bypass_minlsh$Concordance, decreasing=TRUE),]
                       test_bypass_minlsh
                       return(test_bypass_minlsh)}
                       , caption="My Candidates Ranked", rownames=FALSE, escape=FALSE)
                      test_bypass_minlsh <<- test_bypass_minlsh
                      #stopApp()
                    }
                    # cluster_compounds()
